    ...
    while IFS= read -r -d '' track; do
        ...
        if [[ -n "${track_map[$track_key]}" ]]; then
            ...
            if (( track_quality > existing_quality )); then
                ...
                track_map[$track_key]="$target_dir/$(basename "$track")"
            else
...
consolidate_duplicates() {
    ...
    while IFS= read -r -d '' item; do
        ...
        album_key="${norm_artist_name}_${norm_album_name}"

        if [[ -n "${album_map[$album_key]}" ]]; then
            # Check if the current album is a deluxe edition and should be prioritized
            if [[ "$album_name" == *"Deluxe Edition"* ]]; then
                log_message "$MAGENTA" "Removing non-deluxe album: ${album_map[$album_key]}"
                rm -rf "${album_map[$album_key]}"
                album_map[$album_key]="$item"
            else
